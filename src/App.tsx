...
function ThreeViewport() {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog('#0d1115', 11, 24)
    ...
    scene.add(rim)
    const grid = new THREE.GridHelper(12, 24, '#2a3f44', '#17262c')
    ...
    const resize = () => {
      const width = mount.clientWidth || 800; const height = mount.clientHeight || 600
  ...
  useEffect(() => {
    ...
    if (!scene) return
    const stage = scene.children.find((child) => child instanceof THREE.Group) as THREE.Group | undefined
    if (!stage) return
    stage.clear(); meshMapRef.current.clear()
    const dispose = (object: THREE.Object3D) => { object.traverse((child) => { const mesh = child as THREE.Mesh; if (mesh.geometry) mesh.geometry.dispose(); const material = mesh.material as THREE.Material | THREE.Material[] | undefined; if (Array.isArray(material)) material.forEach((item) => item.dispose()); else material?.dispose() }) }
    const makeTextTexture = (label: string, color: string) => { const canvas = document.createElement('canvas'); canvas.width = 900; canvas.height = 180; const context = canvas.getContext('2d')!; context.clearRect(0, 0, 900, 180); context.font = '700 72px Inter, Arial'; context.fillStyle = color; context.fillText(label, 12, 100); const texture = new THREE.CanvasTexture(canvas); texture.colorSpace = THREE.SRGBColorSpace; return texture }
    objects.forEach((object) => {
      let visual: THREE.Object3D
      if (object.kind === 'light') { const light = new THREE.PointLight(object.color, 3.2, 5, 2); light.castShadow = true; visual = light; const bulb = new THREE.Mesh(new THREE.SphereGeometry(0.1, 12, 8), new THREE.MeshBasicMaterial({ color: object.color })); light.add(bulb) }
      else if (object.kind === 'text') { const texture = makeTextTexture('LUMEN / 24', object.color); visual = new THREE.Sprite(new THREE.SpriteMaterial({ map: texture, transparent: true, depthTest: false })); visual.scale.set(2.6, 0.52, 1) }
      else if (object.kind === 'group') { visual = new THREE.Group() }
      else {
        let geometry: THREE.BufferGeometry
        if (object.shape === 'torus') geometry = new THREE.TorusGeometry(1.15, 0.035, 16, 96)
        else if (object.shape === 'plane') geometry = new THREE.PlaneGeometry(2, 2)
        else if (object.shape === 'cone') geometry = new THREE.ConeGeometry(0.85, 1.6, 48)
        else if (object.shape === 'box') geometry = new THREE.BoxGeometry(1.3, 1.3, 1.3, 3, 3, 3)
        else geometry = new THREE.SphereGeometry(0.95, 64, 32)
        const material = new THREE.MeshPhysicalMaterial({ color: object.color, metalness: object.metalness, roughness: object.roughness, transparent: object.opacity < 1, opacity: object.opacity, clearcoat: object.shape === 'sphere' ? 0.6 : 0.15, emissive: object.shape === 'sphere' ? new THREE.Color(object.color).multiplyScalar(0.08) : '#000000' })
        const mesh = new THREE.Mesh(geometry, material); mesh.castShadow = true; mesh.receiveShadow = true; visual = mesh
      }
      visual.name = object.name; visual.userData.objectId = object.id; visual.position.set(...object.position); visual.rotation.set(...object.rotation); visual.scale.set(...object.scale); visual.visible = object.visible; visual.userData.locked = object.locked
      stage.add(visual); meshMapRef.current.set(object.id, visual)
      if (object.id === selectedId && object.visible) { const box = new THREE.BoxHelper(visual, new THREE.Color('#73ebd0')); box.renderOrder = 3; stage.add(box); const points = new THREE.Points(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(-1, -1, -1)]), new THREE.PointsMaterial({ color: '#ffffff', size: 0.1 })); points.visible = false; void points }
    })
    const grid = scene.children.find((child) => child instanceof THREE.GridHelper) as THREE.GridHelper | undefined
    if (grid) grid.visible = showGrid
    return () => { stage.traverse(dispose) }
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      const hits = raycaster.intersectObjects(Array.from(meshMapRef.current.values()), true); const hit = hits[0]?.object
      if (!hit) return
      let current: THREE.Object3D | null = hit; while (current && !current.userData.objectId) current = current.parent
      if (current?.userData.objectId) { const id = current.userData.objectId as string; const object = useEditorStore.getState().objects.find((item) => item.id === id); if (object && !object.locked) selectObject(id) }
    }
    renderer.domElement.addEventListener('pointerdown', onPointerDown)
    renderer.domElement.style.cursor = activeTool === 'hand' ? 'grab' : activeTool === 'select' ? 'default' : 'crosshair'
